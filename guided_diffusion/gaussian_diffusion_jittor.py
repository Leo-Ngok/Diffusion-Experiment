...
from .nn_jittor import mean_flat
from .losses_jittor import normal_kl, discretized_gaussian_log_likelihood
...
class GaussianDiffusion:
    ...
    def p_mean_variance(
        self, model, x, t, clip_denoised=True, denoised_fn=None, model_kwargs:Optional[dict]=None
    ):
        """
        ...
        if 's' in model_kwargs and model_kwargs['s'] > 1.0:
            model_output_zero = model(x, self._scale_timesteps(t), y=jt.zeros_like(model_kwargs['y']))
    ...
    def _prior_bpd(self, x_start):
        ...
        t = jt.var([self.num_timesteps - 1] * batch_size)
...
def _extract_into_tensor(arr: np.ndarray, timesteps, broadcast_shape):
    ...
    """
    res = jt.array(arr)[timesteps].float()#.to(device=timesteps.device)[timesteps].float()
