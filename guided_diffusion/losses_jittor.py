...
def normal_kl(mean1, logvar1, mean2, logvar2):
    ...
    # Tensors, but it does not work for jt.exp().
    logvar1, logvar2 = [
        x if isinstance(x, jt.Var) else jt.tensor(x).to(tensor)
...
def approx_standard_normal_cdf(x):
    ...
    standard normal.
    """
    return 0.5 * (1.0 + jt.tanh(np.sqrt(2.0 / np.pi) * (x + 0.044715 * jt.pow(x, jt.ones(1) * 3))))
