...
class LossAwareSampler(ScheduleSampler):
    def update_with_local_losses(self, local_ts, local_losses):
        ...
        """
        batch_sizes = [
            jt.tensor([0], dtype=jt.int32, device=local_ts.device)
            for _ in range(1)#dist.get_world_size())
        ]
        #dist.all_gather(
        #    batch_sizes,
        #    jt.tensor([len(local_ts)], dtype=jt.int32, device=local_ts.device),
        #)
...
class LossSecondMomentResampler(LossAwareSampler):
    ...
    def weights(self) -> np.ndarray:
        ...
        weights:np.ndarray = np.sqrt(np.mean(self._loss_history ** 2, axis=-1))
