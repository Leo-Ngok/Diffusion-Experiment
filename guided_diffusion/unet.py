...
class UNetModel(nn.Module):
    ...
    def forward(self, x:th.Tensor, timesteps, y:Optional[th.Tensor]=None):
        """
...
class EncoderUNetModel(nn.Module):
    ...
    def forward(self, x:th.Tensor, timesteps):
        """
        Apply the model to an input batch.
        ...
        h = x.type(self.dtype)
        for module in self.input_blocks:
            h:th.Tensor = module(h, emb)
            if self.pool.startswith("spatial"):
                results.append(h.type(x.dtype).mean(dim=(2, 3)))
        h = self.middle_block(h, emb)
        if self.pool.startswith("spatial"):
            results.append(h.type(x.dtype).mean(dim=(2, 3)))
            h = th.cat(results, -1) #axis=-1)
            return self.out(h)
        else:
            h = h.type(x.dtype)
            return self.out(h)
