...
class SPADEGroupNorm(nn.Module):
    ...
    def execute(self, x:jt.Var, segmap: jt.Var) -> jt.Var:
        # Part 1. generate parameter-free normalized activations
        x = self.norm(x)

        # Part 2. produce scaling and bias conditioned on semantic map
        segmap = jt.nn.interpolate(segmap, size=x.size()[2:], mode='nearest')
...
class QKVAttentionLegacy(nn.Module):
    ...
    def execute(self, qkv):
        ...
        weight = jt.nn.softmax(weight.float(), dim=-1)
...
class QKVAttention(nn.Module):
    ...
    def execute(self, qkv: jt.Var) -> jt.Var:
        """
        Apply QKV attention.
        ...
        weight:jt.Var = jt.nn.softmax(weight.float(), dim= -1)
...
class UNetModel(nn.Module):
    ...
    def __init__(
        ...
        out_channels:int,
        num_res_blocks:int,
        attention_resolutions:Union[list, set, tuple],
        dropout=0,
        ...
        for level, mult in enumerate(channel_mult):
            ...
            if level != len(channel_mult) - 1:
                ...
                ds *= 2
                self._feature_size += ch
        channels = num_classes if num_classes is not None else 3
        self.middle_block = TimestepEmbedSequential(
            SDMResBlock(
                ch,
                time_embed_dim,
                dropout,
                c_channels=channels,#num_classes,
                dims=dims,
                use_checkpoint=use_checkpoint,
                use_scale_shift_norm=use_scale_shift_norm,
            ),
    ...
    def execute(self, x:jt.Var, timesteps:jt.Var, y:Optional[jt.Var]=None) -> jt.Var:
        """
        ...
        if self.num_classes is not None:
            assert y is not None
            print('y.shape= ', y.shape, 'x.shape = ', x.shape, 'x.shape[0]=', x.shape[0])
...
class EncoderUNetModel(nn.Module):
    ...
    def execute(self, x:jt.Var, timesteps:jt.Var) -> jt.Var:
        """
        Apply the model to an input batch.
